...
import type { Bundle, FishingLocation, TideData, TaggedAnimal, StockingInfo, OccTaxon } from "./types";
...
export async function loadBundle(loc: FishingLocation, days = 7): Promise<Bundle> {
  ...
  const [weather, marine, tide, predators, stocking, nearbyTaxa] = await Promise.all([
