...
export function assembleBundle(location: FishingLocation, weather: RawWeather, marine: RawMarine, tide: TideData, predators: TaggedAnimal[] = []): Bundle {
  ...
  const hours: HourPoint[] = weather.hours.map((w, i) => {
    ...
    return {
      ...
      wavePeriod: m?.wavePeriod ?? null,
      waveDir: m?.waveDir ?? null,
      swellHeight: m?.swellHeight ?? null,
      swellPeriod: m?.swellPeriod ?? null,
      swellDir: m?.swellDir ?? null,
      windWaveHeight: m?.windWaveHeight ?? null,
      windWavePeriod: m?.windWavePeriod ?? null,
      windWaveDir: m?.windWaveDir ?? null,
      currentVelocity: m?.currentVelocity ?? null,
      currentDir: m?.currentDir ?? null,
